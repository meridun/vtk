...
One `*.toml` file per filter. Each is embedded at build (`//go:embed defs/*.toml`
in the parent package), compiled into a pure `func(raw string) string`, and
registered into the filter dispatch alongside the hand-written Go filter
families. Use TOML for tools whose output can be compacted by dropping/keeping
lines and capping length; keep Go for reformatters that regex-stripping can't do
(spec-tree folding, per-rule rollups).
...
| `[[tests.<name>]]` | `{input, expected}` | inline fixtures, run under `go test` |
...
passes it through untouched (no `OK <id>`), exactly like a Go filter that finds
nothing to compact.
