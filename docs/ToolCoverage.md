...
| rtk subsystems | `learn` (#43), `discover` (#44), `hooks` (#45), `analytics`/gain-economics (#46) | — | in-progress — upstream subsystems beyond filtering; vtk equivalents tracked in #43–#46. The `hooks` Claude-Code MVP shipped via #45 (see the Meta `hooks` row; Copilot installer split to #83, trust/permissions deferred). The log-based half of gain-economics (dollarized `--daily`/`--graph`/`--history` rollups) shipped via #58; the per-session view stays in #46, blocked on the shared session provider (#43/#44) |
